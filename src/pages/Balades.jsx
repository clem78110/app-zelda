...
export default function Balades() {
  ...
  const cleanupTracking = useCallback(() => {
    ...
    if (wakeLock.current) { try { wakeLock.current.release(); wakeLock.current = null; } catch {} }
  ...
  const start = () => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        } else { lastPoint.current = p; }
      },
      (err) => { setGpsError("Erreur GPS"); },
  ...
  const stop = async (save = true) => {
    ...
    if (!activePet) return;
    try {
      await api.post("/walks", { pet_id: activePet.id, distance_km: +finalDistance.toFixed(3), duration_seconds: finalDuration, notes: "" });
      toast.success("Balade enregistrée");
      load();
    } catch (e) { toast.error("Erreur enregistrement"); } finally { setDistance(0); setDuration(0); }
  };

  const remove = async (id) => {
    try { await api.delete(`/walks/${id}`); load(); } catch (e) { toast.error("Suppression impossible"); }
  };
